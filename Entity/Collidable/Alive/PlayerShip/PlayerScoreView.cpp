...
void Entity::PlayerScoreView::generateShape()
{
        ...
        Utils::Vector2D position = Utils::Transformation::getInstance().transAndCenter(
            Utils::Vector2D(0.5, -2.5), Utils::Vector2D(w->getSize().x, w->getSize().y), Utils::Vector2D(1, 1));
        std::dynamic_pointer_cast<sf::Text>(shape)->setPosition(position.x, position.y);
}
void Entity::PlayerScoreView::update()
{
